...
探索本体はWasm内に閉じ込め、JavaScriptとの往復は1手ごとに1回だけです。Web Worker上で実行するため、AIの探索中もUIとアニメーションは停止しません。
...
ゲーム状態と着手履歴は`localStorage`に保存されます。AI対戦時のUndoは、直前の人間とAIの1ラウンドを戻します。
...
solverは最大42手まで探索できますが、Web UIでは応答性を保つためAIの1手あたりの探索時間を3秒に制限しています。そのためUIからの着手は「制限時間内で最も深く探索できた手」です。`apps/web/src/App.tsx`の`timeMs`を`0`にすると時間制限なしの探索になりますが、序盤局面では非常に長くなる場合があります。

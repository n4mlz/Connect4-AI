...
AI対戦中は「AI探索情報」トグルから、直近の反復深化で完了した探索深度と、先手（赤）基準の評価値を確認できます。完全読みでは、両者が最善手を選んだ場合に勝敗が確定する局面の空きマス数を評価値として使います。早い勝利ほど大きく、終盤の勝利ほど小さい値になります。未完全な探索では勝敗を確定させず、空きマス数も表示しません。グラフも完全読みできた評価値だけを描画します。赤を上、後手（黄）を下に配置します。探索が残り局面をすべて読み切った場合は「完全読み」と表示されます。各手の評価値はlocalStorageにも保存され、Undoやリロード後もグラフを復元します。
